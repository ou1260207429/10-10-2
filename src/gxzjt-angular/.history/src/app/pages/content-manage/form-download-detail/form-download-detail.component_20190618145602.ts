...
export class FormDownloadDetailComponent implements OnInit {
  ...
  constructor(private msg: NzMessageService, private _publicServices: PublicServices, private _eventEmiter: EventEmiter, private message: NzMessageService, private _attachmentServiceProxy: AttachmentServiceProxy, private _activatedRoute: ActivatedRoute) {
  }
  ngOnInit() {
  ...
  uploadFiles() {
    ...
    const keys = Object.keys(uploadFileModel);  //获取对象key,['name':'123],这个方法拿到name
    ...
    this._publicServices.upload(JSON.parse(JSON.stringify(uploadFileModel))).subscribe(data => {
      console.log(data)
    })
  }
