...
export class FormDownloadDetailComponent implements OnInit {
  ...
  constructor(@Inject(HttpClient) http: HttpClient, @Optional() @Inject(API_BASE_URL) baseUrl?: string, private _eventEmiter: EventEmiter, private message: NzMessageService, private _attachmentServiceProxy: AttachmentServiceProxy, private _activatedRoute: ActivatedRoute) {
  }
  ngOnInit() {
  ...
  uploadFiles() {
    ...
    _HttpClient.post(this.uploadUrl, this.uploadParams).subscribe(data => {
      console.log(data)
    })
  }
  createguid() {
