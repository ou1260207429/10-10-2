...
var app = {
  ...
  /**
   * post请求
   * url为服务器的地址,data为请求参数,dataType表示请求头格式
   * */
  post: function (url, data, isLoad, dataType, flag, succCallBack, errorCallBack) {
    ...
    return app.ajax(url, formData, "POST", isLoad, dataType, succCallBack, errorCallBack);
  },
   ...
   * */
  ajax: function (url, data, method, isLoad, dataType, succCallBack, errorCallBack) {
    let myLoading;
    if (!isLoad) {
