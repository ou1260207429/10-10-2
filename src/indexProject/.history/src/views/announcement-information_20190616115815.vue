      ...
      <el-row :gutter="8">
        <el-col :span="4">
          <div>
            ...
            >公告信息</div>
            <p
              :class="{activeInfo:index==activeType}"
              @click="getInfo(item.type)"
              :key="index"
              v-for="(item,index) in navList"
              style="background-color: #fff;font-size: 15px;padding: 14px 0;border-bottom: 1px solid #e8e8e8;margin: 0;text-align:center;"
            >{{item.name}}</p>
          </div>
        </el-col>
        <el-col :span="20">
