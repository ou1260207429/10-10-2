    ...
    <div class="content">
      ...
      <el-row :gutter="16"> 
        ...
        <el-card style="height:450px;">
          ...
          <div v-if="lawsList">
             <ul>
              <li v-for="(item,idnex) in lawsList" :key="index">
                <div style="overflow: hidden"> <span class="title">{{item.title}}</span>
                  <span class="time">————{{item.issueOrg}}</span></div>
                <div style="overflow: hidden;margin-top: 6px;"> <span
                    style="float: right;font-size: 8px">{{item.creationTime|timeNewString}}</span></div>
              </li>
            </ul>
          </div>
        </el-card>
