...
export class PoliciesAndRegulationsComponent implements OnInit {
  ...
  disabledStartDate = (startValue: Date): boolean => {
    if (!startValue || !this.params.endTime) {
      return false;
    }
    return startValue.getTime() > this.params.endTime.getTime();
  };

  disabledEndDate = (endValue: Date): boolean => {
    if (!endValue || !this.params.startTime) {
      return false;
    }
    return endValue.getTime() <= this.params.startTime.getTime();
  };
