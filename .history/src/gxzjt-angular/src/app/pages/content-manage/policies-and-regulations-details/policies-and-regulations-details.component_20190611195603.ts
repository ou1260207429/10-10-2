...
export class PoliciesAndRegulationsDetailsComponent implements OnInit {
   ...
   */
  editRegulationAsync(){
    this._regulationServiceProxy.editRegulationAsync(this.data).subscribe(data=>{
   ...
   */
  save(){
    const src = this.operate == 'add'?this._regulationServiceProxy.addRegulationAsync(this.data):this._regulationServiceProxy.editRegulationAsync(this.data)
    console.log(this.data);
