...
export class PoliciesAndRegulationsDetailsComponent implements OnInit {
  ...
  constructor(private _eventEmiter: EventEmiter, private message: NzMessageService, private _regulationServiceProxy: RegulationServiceProxy, private _activatedRoute: ActivatedRoute) {
    this.regulationTypeId = parseInt(this._activatedRoute.snapshot.paramMap.get('id'));
  ...
  initType() {
    let params = {
      regulationId: this.regulationTypeId
    }
  ...
  getRegulationDetailsByIdAsync() {
    this._regulationServiceProxy.getRegulationDetailsByIdAsync(this.regulationTypeId).subscribe((data: any) => {
      this.data = data
      this.data.regulationId = this.regulationTypeId;
